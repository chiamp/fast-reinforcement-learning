...
However, current reinforcement learning methods are very sample inefficient; they require a huge amount of training games in order to learn to play at a reasonable level. To quote the paper: "in order to learn how to play one game of Atari, a simple video game console from the 1980s, an agent typically consumes an amount of data corresponding to several weeks of uninterrupted playing; it has been shown that, in some cases, humans are able to reach the same performance level in around 15 min".

Thus, there is motivation to find more sample efficient algorithms that require less training games in order to reach a reasonable level of performance. This paper proposes some algorithms to address this problem.

## Approach
One hypothesis put forward as to why humans are able to learn a lot faster compared to the AI algorithms today, is because we can draw from our past experiences to help bootstrap, or "jump-start", the learning process of a new task we haven't seen before; therefore requiring less experience and time to learn a new task. This is with the assumption that some of the tasks we've learned to do in the past may share some similar characteristics and features with the new task at hand, and so we don't have to start from scratch when learning this new task.
...
### Tasks
In the traditional RL framework described above, the agent's goal is to maximize the reward it receives. The reward is defined by the dynamics of the environment, and thus you could say that the reward dynamics determines the task the agent must optimize for.

For example, playing a zero-sum board game would have reward dynamics structured such that winning the game would result in receiving a positive reward, while losing the game would result in receiving a negative reward. Therefore given the dynamics of this environment, training an agent to maximize the reward received would essentially train the agent to optimize for the task of developing a strategy that lets it win as much as possible on this board game.

As mentioned before, agents typically train from scratch for every new task at hand. But in this paper, we develop a framework where agents are able to utilize skills it developed from past tasks, in order to help learn optimal strategies for tackling new tasks faster.
...
Therefore given the transition dynamics are the same, we can create an algorithm that emulates this phenomenon. We can train an agent on a set of tasks, and allow it to utilize the skills it learns to help bootstrap and speed up its learning process on a new task. 
...
The task vector used for evaluation was <img src="https://render.githubusercontent.com/render/math?math=[1,-1]">.
...
* Update the reward function <img src="https://render.githubusercontent.com/render/math?math=u"> according to this update rule:
	* <img src="https://render.githubusercontent.com/render/math?math=\min_u[u( \phi_h(s,a,s^') ) - r]^2">
...
The non-linear task used for evaluation was giving the agent a reward of +1 if it picked up the more abundant object, a reward of -1 if it picked up the less abundant object, and 0 otherwise.

---

The same experiment using task vectors and dot product was repeated except on the non-linear tasks mentioned above. The results can be seen in the graph below:
...
## Future Work
Using the generalized policy to bootstrap and learn a new successor feature function for a new task
